...
export default function Feedback() {
  ...
  const [statistics, setStatistics] = useState({
    ...
    ratingStats: Array(5)
      .fill(0)
      .map((_, i) => ({ rating: i + 1, _count: { rating: 0 } })),
  });
  ...
  const totalFeedbacks = statistics.ratingStats.reduce(
    (sum: number, stat: { _count: { rating: number } }) =>
      sum + stat._count.rating,
    0,
  );
      ...
      <div className={"mb-4"}>
        ...
        <Group gap="xs">
          <Text
            size="xl"
            fw={700}
            c={getRatingColor(Math.round(statistics.averageRating))}
          >
