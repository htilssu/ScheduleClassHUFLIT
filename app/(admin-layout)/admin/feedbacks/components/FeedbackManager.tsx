...
import { Button, Card, Text, Group, Stack, Container, Pagination, Flex, Modal, LoadingOverlay, Title, ButtonGroup } from "@mantine/core";
...
const FeedbackManager = () => {
    ...
    const [feedbackToDelete, setFeedbackToDelete] = React.useState<string | null>(null);
    ...
    const { data, isLoading, error } = useQuery<FeedbackResponse, Error>({
        ...
        queryFn: async () => {
            const response = await fetch(
                `/v1/admin/feedbacks?page=${currentPage}&limit=10${selectedRating ? `&rating=${selectedRating}` : ''}`,
                {}
            );
            if (!response.ok) {
    ...
    const handleViewUser = async (user: { id: string; name: string | null; email: string }) => {
    ...
    return (
        <Container size="xl">
            <Stack gap="lg">
                <Card withBorder>
                    <Title order={2} mb="md">Quản lý phản hồi</Title>
                    
                    <Group mb="xl">
                        <Text size="sm" fw={500}>Lọc theo đánh giá:</Text>
                        <ButtonGroup>
                            <Button
                            ...
                            {[5, 4, 3, 2, 1].map((rating) => (
                                <Button
                                    key={rating}
                                    variant={selectedRating === rating.toString() ? "filled" : "light"}
            ...
            >
                <Stack gap="md">
                    ...
                    <Text size="sm" c="dimmed">Hành động này không thể hoàn tác.</Text>
